...
def get_food_plan(**kwargs):
    ...
    url = f"https://www.sw-ka.de/de/essen/?d={date.date().isoformat()}"
    # get page for actual or next week
    page = urllib.request.urlopen(url)
    page_parsed = BeautifulSoup(page, "lxml")
    found = page_parsed.find('div', {"id": f"fragment-c{canteens[mensa]}-1"}).table.children

    foodplan = {}
    for row in found:
        linie = row.find("td", class_="mensatype").div.text
